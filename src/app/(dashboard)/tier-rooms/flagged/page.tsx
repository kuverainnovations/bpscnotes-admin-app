...
export default function FlaggedUsersPage() {
  ...
  // Group flags by user — handles both snake_case and camelCase field names
  const byUser = users.reduce((acc: any, flag: any) => {
    const uid = flag.user_id || flag.userId || flag.id || 'unknown'
    if (!acc[uid]) {
      acc[uid] = {
        user_id:   uid,
        user_name: flag.user_name || flag.userName || flag.name || 'Unknown User',
        createdAt: flag.created_at || flag.createdAt,
        flags: []
      }
    }
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {grouped.map((u: any) => {
              ...
              const name = u.user_name || u.userName || u.name || 'Unknown User'
              return (
