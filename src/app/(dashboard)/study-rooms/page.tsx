...
export default function DashboardPage() {
  ...
  const load = async () => {
    ...
    try {
      // allSettled: partial failure doesn't kill the whole page
      ...
      if (s.status  === 'rejected')  setError((s.reason as Error)?.message ?? 'Failed to load stats')
    } catch (e: any) { setError(e.message) }
    finally { setLoading(false) }
  }

  // BUG 11 FIX: admin is synchronously available — no isLoading dependency needed
  useEffect(() => { if (admin) load() }, [admin]) // eslint-disable-line
  ...
  if (error && !stats) return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="card p-8 text-center max-w-md">
        ...
        {/* BUG 10 FIX: show actual API URL — not "localhost:5000" */}
        <p className="text-slate-400 text-sm mb-1">
          API: <code className="text-xs bg-slate-100 px-1 rounded">
            {process.env.NEXT_PUBLIC_API_URL}
          </code>
        </p>
        <p className="text-slate-400 text-sm mb-4">The server may be temporarily unreachable.</p>
      ...
      <div className="p-6 space-y-5 animate-fade-in">
        {error && stats && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex items-center gap-3">
            <span>⚠️</span>
            <p className="text-sm text-amber-700">Some data failed to load: {error}</p>
            <button onClick={load} className="ml-auto text-xs font-medium text-amber-600 hover:text-amber-800">Retry</button>
          </div>
        )}
          ...
          {cards.map(c => (
            <div key={c.label} className={`card p-4 ${c.color}`}>
              <div className="flex items-center justify-between mb-2">
                <span className="text-2xl">{c.icon}</span><TrendingUp size={13} className="text-green-500"/>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={userChart}>
                <defs><linearGradient id="ug" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="#1565C0" stopOpacity={0.15}/><stop offset="95%" stopColor="#1565C0" stopOpacity={0}/>
                </linearGradient></defs>
          ...
          <div className="card p-5">
            ...
            <ResponsiveContainer width="100%" height={160}>
              <PieChart><Pie data={revPie} cx="50%" cy="50%" innerRadius={45} outerRadius={70} paddingAngle={3} dataKey="amount">
                {revPie.map((_:any,i:number)=><Cell key={i} fill={PIE_COLORS[i%3]}/>)}
              </Pie><Tooltip formatter={(v:number)=>[`₹${formatNumber(v)}`,'Revenue']} contentStyle={{borderRadius:12,fontSize:12}}/></PieChart>
            </ResponsiveContainer>
            <div className="space-y-2 mt-2">{revPie.map((r:any,i:number)=>(
              <div key={i} className="flex items-center justify-between">
                <div className="flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full" style={{background:PIE_COLORS[i%3]}}/><span className="text-xs text-slate-600 capitalize">{r.plan}</span></div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {[{label:'Courses',value:stats?.totalCourses||0,emoji:'📚'},{label:'Notes',value:stats?.totalNotes||0,emoji:'📄'},{label:'Quizzes',value:stats?.totalQuizzes||0,emoji:'🎯'},{label:'Affairs',value:stats?.totalAffairs||0,emoji:'📰'},{label:'Active Jobs',value:stats?.activeJobs||0,emoji:'💼'}].map(s=>(
            <div key={s.label} className="card p-4 flex items-center gap-3"><span className="text-2xl">{s.emoji}</span><div><p className="text-xl font-bold text-slate-900">{s.value}</p><p className="text-xs text-slate-500">{s.label}</p></div></div>
          ))}
        </div>
        <div className="card p-5">
          <h2 className="section-title">Quick Actions</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[{label:'Add Current Affairs',href:'/current-affairs?create=1',emoji:'📰',color:'bg-blue-50 text-blue-700 hover:bg-blue-100'},{label:'Create Quiz',href:'/quizzes?create=1',emoji:'🎯',color:'bg-purple-50 text-purple-700 hover:bg-purple-100'},{label:'Upload Notes',href:'/content/notes?create=1',emoji:'📄',color:'bg-green-50 text-green-700 hover:bg-green-100'},{label:'Send Notification',href:'/notifications?compose=1',emoji:'🔔',color:'bg-orange-50 text-orange-700 hover:bg-orange-100'}].map(a=>(
              <a key={a.label} href={a.href} className={`flex items-center gap-3 p-3 rounded-xl transition-colors ${a.color}`}><span className="text-xl">{a.emoji}</span><span className="text-sm font-semibold">{a.label}</span></a>
            ))}
          </div>
        </div>
      </div>
